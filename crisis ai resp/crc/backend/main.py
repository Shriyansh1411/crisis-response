...
from .ai.schemas import AnalyzeIncidentRequest
from .ai.workflow import CrisisResponseWorkflow
...
@app.post("/incidents/analyze", tags=["AI"])
async def analyze_incident(data: AnalyzeIncidentRequest):
    """Analyze a natural-language report, select services, and dispatch real units."""
    try:
        return await CrisisResponseWorkflow().run(data)
    except Exception as exc:
        dispatch_service._log(f"AI workflow failed: {exc}", "error")
        raise HTTPException(503, "Incident analysis is temporarily unavailable") from exc
